...
const dificultadOrder: Record<string, number> = { 'Alta': 0, 'Media': 1, 'Baja': 2 };
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {activeFilterTags.map((tag, i) => (
                  <span key={i} className="inline-flex items-center gap-1 bg-primary/15 text-primary text-xs px-2.5 py-1 rounded font-medium">
                    {tag.value}
                    <button onClick={() => removeFilterTag(tag.key, tag.value)}>
                      <X size={10} />
                    </button>
                  </span>
                ))}
                <button onClick={() => { setFilters(emptyFilters); setPage(1); }} className="text-xs text-primary hover:underline">
                  Limpiar todo
                </button>
              </div>
            )}
              ...
              <div className="flex items-center justify-between mt-8">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    disabled={page === 1}
                    className="px-3 py-1.5 text-xs rounded border border-border text-muted-foreground hover:text-foreground disabled:opacity-30 transition-colors"
                  >
                    Prev
                  </button>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
                    <button
                      ...
                      className={`w-8 h-8 text-xs rounded font-medium transition-all ${
                        page === n ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'
                      }`}
                    >
                      {n}
                    </button>
